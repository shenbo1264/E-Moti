...
PORTRAIT_EXPRESSIONS = ("neutral", "smile", "thinking", "surprised", "sad", "sleepy")
...
class CharacterGenerationWorkflow:
    ...
    def create_draft(
        ...
        *,
        ...
        _write_json(pack_dir / "portrait_candidate.json", _portrait_candidate_json())
...
def _art_prompts(brief: Mapping[str, object]) -> dict[str, object]:
    ...
    if brief.get("policy") == "local_fanwork":
        ...
        portrait_common = (
            ...
            "Do not use official art, screenshots, logos, copied lines, or exact asset reproduction. "
            f"User-provided source character for private reference: {source}. "
            "transparent background, consistent identity, visual novel standing portrait, "
            "half-body or full-body composition, clean alpha edges. "
            f"Visual keywords: {visual}. Boundaries: {boundaries}."
        )
    else:
        common = (
    ...
    return {
        ...
        "portrait_base": f"Create one original visual novel standing portrait for the character. {portrait_common}",
        "portrait_expressions": {
            expression: (
                f"Create a {expression} expression variant for the same original visual novel standing portrait. "
                f"Keep face, outfit, palette, proportions, camera angle, and alpha edge quality consistent. {portrait_common}"
            )
            for expression in PORTRAIT_EXPRESSIONS
        },
        "portrait_blink": (
            "Create blink frames for the approved neutral portrait only: neutral_open.png, "
            "neutral_half.png, neutral_closed.png. Keep the same head pose, face, outfit, "
            f"palette, and transparent background. {portrait_common}"
        ),
        "sprite_rows": {
...
def _portrait_candidate_json() -> dict[str, object]:
    expressions: dict[str, object] = {
        expression: f"portraits/{expression}_open.png"
        for expression in PORTRAIT_EXPRESSIONS
    }
    expressions["neutral"] = {
    ...
    return {
        ...
        "notes": [
            "Draft scaffold only; image files are not generated by this workflow.",
            "Do not reference these paths from portrait_manifest.json until human approval.",
        ],
...
def _qa_checklist(brief: Mapping[str, object]) -> str:
    lines = [
        ...
        "- [ ] `portrait_candidate.json` remains `candidate` until human visual QA approves the portrait set.",
        "- [ ] `python tools\\art\\validate_portrait_candidates.py portrait_candidate.json --contact-sheet preview\\portrait-candidate-contact-sheet.png` passes after portrait PNGs exist.",
        "- [ ] Do not reference candidate portraits from a runtime portrait_manifest.json before approval.",
...
def _character_card_markdown(brief: Mapping[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Portrait candidates still require generated PNGs, contact-sheet review, and approval.",
        ]
    )
    return "\n".join(lines) + "\n"
