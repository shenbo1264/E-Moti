...
```powershell
python tools\art\validate_portrait_candidates.py artifacts\spirit-sequence-drafts\xingxi_spirit_rebuild\base_portrait_candidate.json --contact-sheet artifacts\spirit-sequence-drafts\xingxi_spirit_rebuild\review\base-contact-sheet.png
...
The near-term art route is the Spirit sequence-frame workflow in `docs\spirit_sequence_frame_sop.md`. It keeps local drafts under ignored `artifacts\spirit-sequence-drafts\`, creates one canonical base portrait first, then generates state-specific micro-frame strips using that base as the identity reference. The AI-video and LivePortrait tools remain fallback/research paths, not the current default route, because provider video frames can drift too much for reliable promotion.
