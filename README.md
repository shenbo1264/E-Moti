...
`create_portrait_video_source_pack.py` writes `artifacts\portrait-video-source\<set_id>\reference`, `gemini_prompt.md`, `video`, `frames`, and `source_pack.json` so Gemini work can be handed off cleanly. `batch_process_portrait_video_source_packs.py` scans those folders and reports `ready`, `waiting_for_frames`, or processed status; add `--process-ready` to process only source packs that already contain exported PNG frames. `process_portrait_video_source_pack.py` turns one source pack into a motion candidate using the saved prompt as provenance. `extract_portrait_motion_frames.py` is the lower-level extractor; it also accepts `--video` when `ffmpeg` is installed locally.
