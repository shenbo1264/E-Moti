...
`create_portrait_video_source_pack.py` writes `artifacts\portrait-video-source\<set_id>\reference`, `gemini_prompt.md`, `video`, `frames`, and `source_pack.json` so Gemini work can be handed off cleanly. After exported PNG frames are placed under that source pack's `frames` directory, `process_portrait_video_source_pack.py` turns the source pack into a motion candidate using the saved prompt as provenance. `extract_portrait_motion_frames.py` is the lower-level extractor; it also accepts `--video` when `ffmpeg` is installed locally.
