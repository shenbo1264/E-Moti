...
- Latest committed checkpoint before the normalized next-command package: `2597870 feat: cover readiness attention reason gaps`
...
Latest focused AI-video source-pack import tests run on 2026-06-09:

```powershell
python -m pytest tests\test_portrait_video_source_pack_import.py tests\test_portrait_video_source_pack.py tests\test_portrait_video_source_pack_processing.py tests\test_portrait_video_frame_preflight.py tests\test_repository_hygiene.py -q
```
...
- `P3/P5-release-readiness-workflow-commands` package:
...
- `P3-frame-visual-qa-sheet` package:
...
- `P3/P5-release-readiness-frame-visual-qa` package:
...
- `P3/P5-release-readiness-regeneration-brief` package:
  - Extends `tools/release_readiness_report.py` with repeatable `--portrait-regeneration-brief-report` inputs.
  - Release readiness can now show the regeneration decision, frame status, max body drift, blockers, paste-ready retry/negative prompts, prompt constraints, and suggested local commands.
  - This is offline report aggregation only. It does not call providers, edit frames, create motion candidates, update runtime manifests, or approve generated assets.
- `P3-workflow-regeneration-brief-command` package:
...
- `P3-workflow-retry-handoff-command` package:
  - Makes `tools/art/inspect_portrait_video_workflow.py` emit a matching `bundle_portrait_video_retry_handoff.py` command when a source pack needs `review_frame_warnings`.
  - The workflow suggested command chain now routes warning frames through visual QA, regeneration brief, retry handoff zip, and frame preflight.
  - This is report guidance only. It does not run visual QA, create the brief or zip automatically, call providers, edit frames, create motion candidates, update runtime manifests, or approve generated assets.
- `P3/P5-release-readiness-retry-handoff` package:
  - Makes `tools/release_readiness_report.py` accept `--portrait-retry-handoff-report` so the ignored retry handoff zip path, reference image path, and regeneration brief path are visible in release readiness.
  - The release report can now explain that a manual provider upload bundle exists without treating it as generated video success or motion asset approval.
  - This is offline report aggregation only. It does not call providers, upload files, edit frames, create motion candidates, update runtime manifests, or approve generated assets.
- `P3/P5-release-readiness-frame-preflight` package:
...
- `P3/P5-release-readiness-frame-normalization` package:
...
- `P3/P5-source-handoff-zip-content-gate` package:
  - Makes `tools/release_readiness_report.py` inspect provider-neutral handoff zip contents before reporting `portrait_video_handoff` as ready.
  - Required entries are the handoff README, Gemini prompt, provider prompts, source-pack metadata, and at least one `reference/` image entry.
  - This is offline report verification only. It does not create zips, call providers, upload files, edit frames, update runtime manifests, or approve generated assets.
- `P3/P5-source-pack-content-gate` package:
...
- `P3/P5-regeneration-brief-file-reference-gate` package:
  ...
  - This is offline report verification only. It does not create previews, edit frames, create motion candidates, call providers, update runtime manifests, or approve generated assets.
- `P3-provider-video-source-pack-import` package:
...
- `P3-batch-process-report-link` package:
...
- `P5-release-readiness-attention-reason-coverage` package:
  ...
  - This is report explainability only. It does not change readiness decisions, process frames, call providers, edit manifests, or approve generated assets.
- `P3-normalized-source-next-command` package:
  - Rewrites the cloned normalized source pack's `next_command` during frame normalization so it points to the normalized source pack and normalized motion output.
  - This prevents a lower-resolution original source pack command from being copied into the normalized sibling after canvas-size repair.
  - This is local metadata hygiene only. It does not process frames, call providers, edit runtime manifests, approve generated assets, or change release readiness decisions.
