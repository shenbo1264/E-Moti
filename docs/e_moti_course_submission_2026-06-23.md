...
语音页提供 TTS 与 ASR 的配置入口。当前提交版已经具备每角色 voice profile 元数据和 TTS/ASR 通道，但需要诚实说明：提交包没有把伊卡洛斯、奶龙的原始配音训练成果或第三方已训练模型一起打包进去。现在展示的是角色声线方向和可接入接口，不应写成已经完成原配音级别的角色音色复刻。

当前三角色语音方向如下：

| 角色 | 当前状态 | 声线方向 |
| --- | --- | --- |
| 星汐 | 原创角色，可继续从零设计专属声线 | 温柔、清亮、有陪伴感 |
| 伊卡洛斯 | 已有 voice profile 元数据，尚未打包原配音训练成果 | 安静、低情绪起伏、直白 |
| 奶龙 | 已有 voice profile 元数据，尚未打包原配音训练成果 | 呆萌、短句、轻喜剧感 |

下一版语音应优先走成熟方案，而不是从零造轮子：

- TTS：保留 Qwen3TTS/CosyVoice/GPT-SoVITS 等本地或本地服务化方案入口。
- 星汐：可以继续做原创声线，从少量参考描述和人工试听迭代出专属声音。
- 伊卡洛斯、奶龙：更适合用已有音源训练，或接入别人已经训练好的角色音色模型，再把模型作为本地 TTS 服务接进 voice profile。
- ASR：在保留当前 MIC/开始录音按钮的同时，增加“自定义快捷键”设置。玩家按下自己设置的快捷键即可开始或停止录音，把识别文本送入对话输入，从而更自然地和桌宠说话。
...
- ASR 快捷键：增加用户可配置快捷键，一键开始/停止语音输入，让桌宠沟通更接近自然对话。
