...
> 课题页核验：2026-06-23 已通过 Chrome 登录态读取课程页。页面标题为“课题四：赛博宠物游戏研发设计”，分类为游戏策划类。本文按页面要求，把项目整理成“可运行、可演示、可复现、可解释”的提交材料。

## 课题要求对应关系

课程页要求不是复刻经典宠物游戏，也不是做纯视觉小说或工具 app，而是围绕“赛博宠物”做一款可运行体验的完整作品。页面重点可以概括为：

| 课程页要求 | E-Moti 对应实现 |
| --- | --- |
| 做一个可运行的赛博宠物游戏，平台不限，需顺利运行且无明显 bug | 已提供 Windows 冻结应用 `dist/E-Moti/E-Moti.exe` 和安装包 `dist/installer/E-Moti_Setup_0.1.0.exe`，并通过全量测试、冻结 smoke 与 Windows build validator |
| 围绕宠物题材设计核心情感体验与长期陪伴机制 | 角色有心情、信任、关系、回忆、目标、近期事件和反馈文本；AI 表达只增强陪伴感，不接管养成状态 |
| 构建喂养、成长、互动、反馈等基础玩法循环 | 已实现互动动作、商店、背包、投喂/赠送、金币、等级、目标进度、回忆和状态反馈 |
| 在不直接复刻经典作品的前提下注入新的创意表达 | 使用“桌面 AI 伴侣电子宠物”方向，强调透明桌宠窗口、托盘生命周期、多角色切换和 LLM 表演层 |
| 平衡游戏性与易上手，避免变成纯工具或纯叙事展示 | 控制面板提供清晰导航，桌宠模式可直接演示；学习/休息只是动作状态，产品定位仍是电子宠物 |
| 借助 AI 提升原型制作、内容生成或迭代优化效率 | LLM、屏幕感知、搜索、TTS、ASR 都作为可选表达增强；文档中如实说明 AI 使用方式和边界 |
| 说明材料需简洁介绍游戏内容与团队分工，阅读时间约 5 分钟内，格式不限 | 本文档提供 Markdown 和 Obsidian Better Export PDF 两种版本，包含玩法、截图、架构、验证记录和演示顺序 |
| 作业中需说明哪些部分借助 AI、用了什么工具、以什么方式使用 | 本文在功能、技术架构和验证记录中说明 AI 负责表达增强、调研/生图/文档辅助，本地状态机仍负责养成玩法 |
...
| --- | --- |
| Git 分支 | `main` |
| 课程页核验 | 已通过 Chrome 登录态读取并补充课题要求对应关系 |
| 可见角色 | 星汐、伊卡洛斯、奶龙 |
