...
本轮体验优化后，TTS 已增加确定性缓存和后台合成队列：第一次合成仍由本地模型生成，重复演示台词会直接命中缓存，避免课堂演示时按钮长时间像“卡住”。ASR 侧已把 SenseVoice/FunASR 本地服务部署到短路径用户目录，并固定 PyTorch/torchaudio CPU 匹配版本，避免中文深路径和版本错配导致服务无法启动。
...
- ASR：当前已在保留 MIC/开始录音按钮的同时，增加“自定义快捷键”设置。玩家按下自己设置的快捷键即可开始或停止录音，把识别文本送入对话输入；开启自动发送时，识别文本会以 `DialogueRequest(source="asr")` 进入正常对话流程。本轮用本地 Qwen3TTS 生成的中文 wav 做过 SenseVoice live smoke，ASR 成功返回文本“星西测试一下语音识别”，同音字误差存在，但通道已经从端口连通推进到真实转写可用。
...
| 全量测试 | `961 passed` |
| UI/桌宠回归测试 | `108 passed` |
| 角色包校验 | 四个内置包均 `ok=true`；提交展示使用三角色 |
| Pixel-pet pack 校验 | 星汐、伊卡洛斯、奶龙均 `ok=true` |
| 三服务语音 preflight | Qwen3TTS、GPT-SoVITS、SenseVoice ASR 均 `ok=true` |
| Windows 冻结应用 | 已重建，`dist/E-Moti/E-Moti.exe`，5 秒控制面板与桌宠模式 smoke 通过 |
| Windows 安装包 | 已重建，`dist/installer/E-Moti_Setup_0.1.0.exe`，三角色资源验证通过 |
...
- 全量测试：`961 passed`。
- UI/桌宠回归测试：`108 passed`。
- 三角色统一语音入口 live smoke：星汐、伊卡洛斯、奶龙均通过，报告中的角色 profile provider 均为 `http_emoti_voice`；重复演示台词命中 TTS 缓存，单次报告耗时约 0.007 秒。
- 三服务语音 preflight：Qwen3TTS、GPT-SoVITS、SenseVoice ASR 均通过；ASR live smoke 使用合成 wav 返回可读中文文本。
...
- ASR 快捷键打磨：已增加用户可配置快捷键，后续重点是实机麦克风输入下的延迟、误触提示和录音状态反馈。
- 语音服务一键化：把 Qwen3TTS、GPT-SoVITS、SenseVoice ASR 三个本地服务整理成一个更直观的启动/预检面板，减少演示前手动开服务的成本。
