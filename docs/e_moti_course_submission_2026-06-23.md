...
语音页提供 TTS 与 ASR 的配置入口。2026-06-24 的提交候选版已经把伊卡洛斯从原先的 Qwen preset 声线升级为本地 GPT-SoVITS trained voice profile：项目侧可以通过 `http_gptsovits` provider 调用 9882 本地语音服务，角色包内带有生成参考音频和匹配 reference text，实测短句与中句均能由角色 profile 直接合成 WAV。星汐和奶龙仍保留现有可用 voice profile，后续可以按同一流程继续训练专属声线。
...
| 伊卡洛斯 | 已接入 GPT-SoVITS 本地训练声线，profile_id 为 `ikaros_pixel_pet_gptsovits_curated160_e4_v1` | 安静、低情绪起伏、直白 |
